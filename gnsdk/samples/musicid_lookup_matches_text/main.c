...
main( int argc, char* argv[] )
{
	...
	if (argc == 4)
	{
		...
		if (0 == rc)
		{
			...
			_do_sample_text_query( "Supernatural", "Africa Bamba", "Santana", user_handle );

			/* Perform a sample text query with only the Artist name as an input */
			_do_sample_text_query( GNSDK_NULL, GNSDK_NULL, "Phillip Glass", user_handle );
	...
	)
{
	...
	{
		_display_error(__LINE__, "gnsdk_manager_gdo_child_get(GNSDK_GDO_CHILD_NAME_OFFICIAL)", error); 	
	}
}
	...
	)
{
	...
	if (GNSDK_SUCCESS != error)
	{
		_display_error(__LINE__, "gnsdk_manager_gdo_get_type()", error);
	}
	else
	{
	...
	)
{
	...
	if (GNSDK_SUCCESS == error)
	{
		if ((GNSDK_SUCCESS == error) && (album_title != GNSDK_NULL))
		{
			...
			if (GNSDK_SUCCESS != error)
			{
				_display_error( __LINE__, "gnsdk_musicid_query_set_text(ALBUM) failed", error );
			}
		...
		if ((GNSDK_SUCCESS == error) && (track_title != GNSDK_NULL))
		{
			...
			if (GNSDK_SUCCESS != error)
			{
				_display_error( __LINE__, "gnsdk_musicid_query_set_text(TITLE) failed", error );
			}
		...
		{
			error = gnsdk_musicid_query_set_text( query_handle, GNSDK_MUSICID_FIELD_ARTIST, artist_name );
			if (GNSDK_SUCCESS != error)
			{
				_display_error( __LINE__, "gnsdk_musicid_query_set_text(ARTIST) failed", error );
			}
	...
	if (GNSDK_SUCCESS == error)
	{
		error = gnsdk_musicid_query_find_matches( 
					query_handle, 
					&response_gdo 
					);
		if (GNSDK_SUCCESS != error)
		{
			_display_error( __LINE__, "gnsdk_musicid_query_find_matches() failed", error );
		}
	}
